...
<section class="bg-white rounded-3xl shadow-lg p-12 max-w-4xl w-full mx-auto" style="box-shadow: 0 4px 24px rgba(139,90,140,0.08);">
  <h1 class="text-primary text-3xl mb-2 text-center font-fumofumo">网站列表管理</h1>
  <p class="text-muted text-center mb-8">新增、编辑、删除网站条目（标题、描述、链接）</p>

  <h3 class="text-primary text-xl mb-2">新增网站</h3>
  <form method="post" action="<?php echo $basePath; ?>/admin/save_items.php" class="space-y-3">
    <?php echo csrfField(); ?>
    <input type="hidden" name="type" value="websites" />
  ...
  <h3 class="text-primary text-xl mb-2">已有网站</h3>
  <div class="space-y-4">
    <?php foreach ($items as $i => $it): ?>
      <form method="post" action="<?php echo $basePath; ?>/admin/save_items.php" class="bg-gradient-to-br from-gray-50 to-pink-50 rounded-2xl p-4 border border-gray-200 grid grid-cols-1 md:grid-cols-4 gap-3">
        ...
        <div class="flex items-center gap-2">
          ...
          <a class="bg-white border border-gray-300 rounded px-4 py-2 text-muted no-underline" href="<?php echo $basePath; ?>/admin/delete_item.php?type=websites&index=<?php echo $i; ?>&csrf=<?php echo urlencode(csrfToken()); ?>">删除</a>
        </div>
      </form>
    <?php endforeach; ?>
  </div>
